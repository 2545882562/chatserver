...
void doLoginResponse(json &responsejs)
{
    ...
    else // 登录成功
    {
        ...
        if (responsejs.contains("offline_message"))
        {
            vector<string> vec = responsejs["offline_message"];
            for (string &str : vec)
            {
                json js = json::parse(str);
                // time + [id] + name + " said: " + xxx
                if (CHAT_ONE_MSG == js["msgid"].get<int>())
                {
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        if (CHAT_ONE_MSG == msgtype)
        {
            cout << js["time"].get<string>() << " [" << js["fromid"] << "]" << js["fromname"].get<string>()
                 << " said: " << js["msg"].get<string>() << endl;
            continue;
        }
